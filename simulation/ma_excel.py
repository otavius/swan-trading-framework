...
def add_chart(pair, cross, df, writer):
    ...
    # TypeError: Worksheet.insert_chart() missing 1 required positional argument: 'chart'
...
def prepare_data(df_ma_res, df_ma_trades):
    df_ma_res.sort_values(by=["pair", "total_gains"], ascending=[True, False], inplace=True)
...
def create_excel(df_ma_res, df_ma_trades, granularity):
    file_name = f"ma_sim_{granularity}.xlsx"
