...
def prepare_data(df: pd.DataFrame, slow, fast, signal, ema):
    df_an = df.copy()
    df_an = MACD(df_an, slow=slow, fast=fast, signal=signal)
...
def get_sim_pairs(list_of_currencies, ic: InstrumentCollection):
    pairs = []
    for p1 in list_of_currencies:
        for p2 in list_of_currencies:
            pair = f"{p1}_{p2}"
            if pair in ic.instruments_dict.keys():
                pairs.append(pair)
    return pairs
