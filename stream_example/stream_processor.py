...
class PriceProcessor(StreamBase):
    ...
    def __init__(self, shared_price, price_lock: threading.Lock, price_events, log_name, pair, work_queue: Queue):
        super().__init__(shared_price, price_lock, price_events, log_name)
        self.pair = pair 
        self.work_queue = work_queue
        
    def process_price(self):
        price = None
       # price_locks = {} // for alot of streaming data
        
        try:
            self.price_lock.acquire()
            price = copy.deepcopy(self.shared_price[self.pair])
